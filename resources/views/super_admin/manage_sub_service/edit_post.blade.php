        ...
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="card-title">Edit Sub Service</p>
                    <?php } else {?>
                        <p class="card-title">Add Sub Service</p>
                        <?php }?>
                    <div class="card-description pb-5">
                        <a class="btn btn-primary float-right btn-icon-text" href="{{route('manage-sub-service')}}"> <i class="mdi mdi-arrow-left"></i> Back</a>
                    </div>
                    <div class="row">
                        <div class="col-12 col-md-8 offset-md-2">
                            <form class="forms-sample" method="post" action="javascript:void(0)" id="add_company">
                                ...
                                <div class="form-group">
                                 ...
                                 <?php if($sub_services->count() > 0) { ?>
                                    <div class="form-group">
                                        <label for="sub_service_id">Type</label>

                                        <select class="form-control" id="type" name="type" required>
                                            <?php foreach ($sub_services as $sub) {
                                            ?>
                                                <option value="{{@$sub->type}}" <?php if(@$sub->type == @@$blog->type) { echo "selected"; } ?>> {{@$sub->type}} </option>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="sub_service_id" name="sub_service_id" required>
                                        <?php foreach ($sub_services as $sub) {
                                        ?>
                                            <option value="{{@$sub->id}}" <?php if(@$sub->id == @@$blog->sub_service_id) { echo "selected"; } ?>> {{@$sub->title}} </option>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea type="text" required name="short_description" rows="10" class="form-control m-input m-input--square"><?php echo @$blog->short_description;  ?></textarea>
                                </div>
                                <div class="form-group">
                                    ...
                                    <textarea id="editor" name="description" style="display: none;"><?php echo @$blog->description;  ?></textarea>
                                </div>
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if(response.status) {
                               ...
                               window.location.href = ajax_url+"/manage-sub-service";
