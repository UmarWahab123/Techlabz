...
//            print_r($uploadPath); exit;
    ...
    public function managePost() {
        $data['page_title'] = "Manage Post";
        $data['results'] = Post::where('type','post')->orderBy('id','desc')->get();
        return view('super_admin.manage_post.index',compact('data'));
    }
    public function posts($id = -1) {
        $data['page_title'] = "Add Post";
        if ($id != -1) {
            $data['page_title'] = "Update Post";
            $data['results'] = Post::where('id',$id)->first();
        }
        return view('super_admin.manage_post.save',compact('data'));
    }
    ...
    { 
        ...
        $affected_rows = $request->id ? Post::find($request->id)->update($data) : Post::create($data);
    
        Session::put('message', set_message($affected_rows, 'Post', $action));
    ...
    public function update_post_status(Request $request){
        ...
        $rowsUpdated = Post::where('id',$id)->update(['status'=>$status]);
    ...
    public function oldupdate_post_status(Request $request)
    {
        if (!empty($_POST)) {
            $data  = $request->all();
    ...
    {
     $affected_rows = Post::find($id)->delete();
     $message = set_message($affected_rows,'Post','Deleted');
     Session::put('message', $message);
