...
class RepairContactController extends Controller
{
    ...
    public function manageContact(Request $request) {
        return view('super_admin.manage_repair_contact.manage_post');
    }

    public function editContact($id) {
        $data = array();
        $data['blog'] = RepairContact::where('id',$id)->with(['messages'])->first();
        $con = RepairContact::find($id);
        $con->status = 1;
        $con->save();
        return view('super_admin.manage_repair_contact.edit_post',$data);
    }
    ...
    public function saveContact(Request $request)
    {
        if (!empty($_POST)) {
            ...
            if ($validator->fails()) {
                ...
                $attachment = RepairContact::find($data['id']);
                ...
                $contact = RepairContact::find($data['id']);
    ...
    {
        $data = RepairContact::orderBy('updated_at','desc')->get();
        ...
        ->editColumn('title',function($data) {
            ...
            return date('Y-m-d g:i A',strtotime($data->created_at));
                ...
                $status = '<ul class="list-inline m-0">
                                 ...
                                 <li class="list-inline-item mail-unread mr-2 ml-1"><a href="'.url("/").'/contact-service/view-contact/'.$data->id.'" class="action-icon font-medium-3" ><i class="mdi mdi-eye" ></i></a></li>
    ...
    public function delete_contact(Request $request)
    {
        if (!empty($_POST)) {
            ...
            RepairContact::where('id',$data['id'])->delete();
